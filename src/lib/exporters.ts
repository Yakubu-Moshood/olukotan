...
function pdfGeometry(context: ExportContext) {
  const letter = context.projectData.screenplaySettings.pagination.pageSize === "US Letter";
  const width = letter ? 612 : 595, height = letter ? 792 : 842;
  return { width, height, contentLeft: 72, contentRight: width - 72, contentTop: height - 72 };
}
function pdfElementX(element: ScreenplayElement, text: string, contentLeft: number, contentRight: number) {
  if (element.type === "transition") return Math.max(contentLeft, contentRight - [...cleanFormatting(text)].length * 6.6);
  return element.type === "character" ? 260 : element.type === "dialogue" ? 170 : element.type === "parenthetical" ? 210 : contentLeft;
}
const pdfExporter: Exporter = { format: "pdf", extension: "pdf", async export(context) {
  const geometry = pdfGeometry(context);
  ...
  if (context.options.includeTitlePage) { const fields = titleFields(context); pages[0].push(`BT /F1 20 Tf ${geometry.width * .3} ${geometry.height * .6} Td (${pdfEscape(context.project.manifest.title)}) Tj ET`, `BT /F1 12 Tf ${geometry.width * .37} ${geometry.height * .54} Td (${pdfEscape(fields.author ?? context.project.manifest.author)}) Tj ET`); pages.push([]); }
  for (const element of visibleElements(context)) { if (element.type === "page-break") { pages.push([]); line = 0; continue; } const text = exportText(context, element), x = pdfElementX(element, text, geometry.contentLeft, geometry.contentRight); const y = geometry.contentTop - line * 14; const number = context.options.includeSceneNumbers && element.type === "scene-heading" ? sceneNumber(element, context.projectData) : undefined; if (number) { const position = context.projectData.screenplaySettings.sceneNumbers.position; if (position !== "right") pages.at(-1)!.push(`BT /F1 10 Tf 38 ${y} Td (${pdfEscape(number)}) Tj ET`); if (position !== "left") pages.at(-1)!.push(`BT /F1 10 Tf ${geometry.width - 60} ${y} Td (${pdfEscape(number)}) Tj ET`); } add(`BT /F1 11 Tf ${x} ${y} Td (${pdfEscape(text)}) Tj ET`, element.type === "scene-heading" ? 2 : 1); }
  ...
  pages.forEach((commands, index) => { const pageNo = 4 + index * 2, contentNo = pageNo + 1, scriptPage = index - titleOffset + 1; kids.push(`${pageNo} 0 R`); if (context.options.includePageNumbers && pageSettings.enabled && scriptPage >= startPage && !(pageSettings.hideOnFirstPage && scriptPage === 1)) { const position = pageSettings.position; const x = position === "top-centre" ? geometry.width / 2 : geometry.width - 55, y = position === "bottom-right" ? 35 : geometry.height - 30; const visibleNumber = pageSettings.firstVisibleNumber + scriptPage - startPage; commands.push(`BT /F1 10 Tf ${x} ${y} Td (${visibleNumber}) Tj ET`); } const stream = commands.join("\n"); objects[pageNo - 1] = `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${geometry.width} ${geometry.height}] /Resources << /Font << /F1 3 0 R >> >> /Contents ${contentNo} 0 R >>`; objects[contentNo - 1] = `<< /Length ${stream.length} >>\nstream\n${stream}\nendstream`; }); objects[1] = `<< /Type /Pages /Kids [${kids.join(" ")}] /Count ${pages.length} >>`;
