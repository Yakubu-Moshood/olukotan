...
function readableText(context: ExportContext) {
  return visibleElements(context).map((element) => {
    const text = cleanFormatting(element.text); const number = context.options.includeSceneNumbers && element.type === "scene-heading" ? sceneNumber(element, context.projectData) : undefined;
...
const markdownExporter: Exporter = { format: "md", extension: "md", async export(context) {
  const body = visibleElements(context).map((element) => {
    const text = element.text; const number = context.options.includeSceneNumbers && element.type === "scene-heading" ? sceneNumber(element, context.projectData) : undefined;
    if (element.type === "scene-heading") return `## ${number ? `${number} Â· ` : ""}${text}`;
...
const htmlExporter: Exporter = { format: "html", extension: "html", async export(context) {
  const body = visibleElements(context).map((element) => {
    ...
    return `<div class="${element.type}">${number ? `<span class="scene-number left">${escapeXml(number)}</span>` : ""}${escapeXml(element.text)}${number ? `<span class="scene-number right">${escapeXml(number)}</span>` : ""}</div>`;
  }).join("\n");
...
const fdxExporter: Exporter = { format: "fdx", extension: "fdx", async export(context) {
  const paragraphs = visibleElements(context).filter((element) => element.type !== "page-break").map((element) => {
    const number = context.options.includeSceneNumbers && element.type === "scene-heading" ? sceneNumber(element, context.projectData) : undefined;
    return `<Paragraph Type="${fdxType[element.type] ?? "Action"}"${number ? ` Number="${escapeXml(number)}"` : ""}${element.metadata?.sceneId ? ` SceneId="${escapeXml(element.metadata.sceneId)}"` : ""}><Text>${escapeXml(element.text)}</Text></Paragraph>`;
  }).join("");
